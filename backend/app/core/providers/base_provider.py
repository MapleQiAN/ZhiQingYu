...
- 目标是让用户多说，你多问，通过提问来探索用户的烦恼点在哪里
"""
        elif conversation_stage == "exploring":
            stage_specific_instruction = """
【当前阶段：情绪与事件探索（阶段2）- 提问探索】
你的任务是：
...
- 保持探索性，让用户多说，你多问，通过提问来深入了解
"""
        elif conversation_stage == "summarizing":
            stage_specific_instruction = """
【当前阶段：小结与校准（阶段3）- 提问确认】
你的任务是：
- 简单复述用户刚才分享的核心内容（2-3句话即可），不要添加任何分析或结论
- 例如："今天困扰你的主要是……"、"从刚才的对话中，我听到……"
- 然后提问邀请用户校正，例如："这些有说到你心里吗"、"有没有哪里我理解得不对，或者漏掉了什么"、"还有什么是你觉得重要的吗"
...
重要要求（引导阶段 - 提问探索，不生成结论）：
- 你的回复应该简洁、直接，不要过于冗长
- 重点在于通过提问来探索和收集信息，明确用户的烦恼点在哪里
- 回复要自然、流畅，保持对话的节奏感
- 以提问和简短共情为主，避免长篇大论
- 严格禁止：不要生成任何结论、分析、解释、建议或诊断
- 严格禁止：不要对用户的问题进行深入分析或给出专业见解
- 严格禁止：不要提前给出解决方案或行动建议
- 你的目标是：让用户多说，你多问，通过提问来探索和收集信息
